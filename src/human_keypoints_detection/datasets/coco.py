...
# 过滤掉所有关于JPEG文件的警告
warnings.filterwarnings("ignore", message=".*Premature end of JPEG file")
...
class CocoTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        filename = os.path.join(self._images_folder, label['img_paths'])
        if not os.path.exists(filename):
            return self.__getitem__(idx + 1)  # 图像不存在尝试加载下一张图像
