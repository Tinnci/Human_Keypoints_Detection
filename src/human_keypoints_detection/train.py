...
from human_keypoints_detection.datasets.coco import CocoTrainDataset
from human_keypoints_detection.datasets.transformations import ConvertKeypoints, Scale, Rotate, CropPad, Flip
from human_keypoints_detection.modules.get_parameters import get_parameters_conv, get_parameters_bn, get_parameters_conv_depthwise
from human_keypoints_detection.models.with_mobilenet import PoseEstimationWithMobileNet
from human_keypoints_detection.modules.loss import l2_loss
from human_keypoints_detection.modules.load_state import load_state, load_from_mobilenet
from human_keypoints_detection.val import evaluate
    ...
    for epochId in range(current_epoch, 280):
        ...
        for batch_data in train_loader:
            ...
            if num_iter % log_after == 0:
                print(f"Epoch: {epochId}, Iter: {num_iter}")
                for loss_idx in range(len(total_losses) // 2):
            ...
            if num_iter % checkpoint_after == 0:
                ...
                torch.save({'state_dict': net.module.state_dict(),
                           ...
                           snapshot_name)
                print("Parameters saved at {}".format(snapshot_name))
            if num_iter % val_after == 0:
                print('Validation...')
                evaluate(val_labels, val_output_name, val_images_folder, net)
                print("Validation result saved at {}!".format(val_output_name))
                net.train()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prepared-train-labels', type=str, required=False,
                        default='prepared_train_annotation.pkl',
                        help='path to the file with prepared annotations')
    parser.add_argument('--train-images-folder', type=str, required=False,
    ...
    parser.add_argument('--from-mobilenet', action='store_true', required=False,
    ...
    #保存到autodl-tmp
    checkpoints_folder = '{}/{}_checkpoints'.format(result_folder, args.experiment_name)
    if not os.path.exists(checkpoints_folder):
        os.makedirs(checkpoints_folder)

    validation_folder = '{}/Validation'.format(result_folder, args.validation_folder)
    if not os.path.exists(validation_folder):
        os.makedirs(validation_folder)

    val_output_name = '{}/{}'.format(validation_folder, args.val_output_name)
    ...
    train(args.prepared_train_labels, args.train_images_folder, args.num_refinement_stages, args.base_lr, args.batch_size,
          ...
          checkpoints_folder, args.log_after, args.val_labels, args.val_images_folder, val_output_name,
          args.checkpoint_after, args.val_after, args.model)
